...
const char *pci_device_class_name(u32 code)
{
	...
	result = convert_num_to_name(code,
				     (num_to_name_table *)
				     pci_dev_class_name_table, pdc_max_indx,
				     NULL);

	if (result == NULL) {
		const int pallpi_max_indx =
		    sizeof(pci_all_prg_intfcs_table) /
		    sizeof(num_to_name_table);

		result = convert_num_to_name(code >> 8,
					     (num_to_name_table *)
...
const char *pci_code_type_name(u8 code)
{
	...
	result = convert_num_to_name((u32) code,
				     (num_to_name_table *)
				     pci_code_type_name_table, pct_max_indx,
